<p>
	<label for="<?php echo $title['id'] ?>"><?php _e( 'Title:', 'engine' ) ?></label>
...
<p>
	...
	?> />
	<label for="<?php echo $events_seek_type['id']; ?>_events"><?php _e( 'Number of events to show:', 'engine' ) ?></label>
...
<p>
	...
	?> />
	<label for="<?php echo $events_seek_type['id']; ?>_days"><?php _e( 'Number of days to show:', 'engine' ) ?></label>
...
<p class="ai1ec-limit-by-container">
	...
	<label for="<?php echo $limit_by_cat['id'] ?>"><?php _e( 'Events with these <strong>Categories</strong>', 'engine' ) ?></label>
</p>
...
<p class="ai1ec-limit-by-container">
	...
	<label for="<?php echo $limit_by_tag['id'] ?>"><?php _e( '<strong>Or</strong> events with these <strong>Tags</strong>', 'engine' ) ?></label>
</p>
<div class="ai1ec-limit-by-options-container" <?php if( ! $limit_by_tag['value'] ) { ?> style="display: none;" <?php } ?>>
	...
	<select id="<?php echo $event_tag_ids['id'] ?>" class="ai1ec-widget-tag-ids" name="<?php echo $event_tag_ids['name'] ?>[]" size="5" multiple="multiple">
		...
		<?php if( count( $event_tag_ids['options'] ) == 0 ) : ?>
			<option disabled="disabled"><?php _e( 'No tags found.', 'engine' ) ?></option>
		<?php endif ?>
	</select>
</div>
<p class="ai1ec-limit-by-container">
	...
	<label for="<?php echo $limit_by_post['id'] ?>"><?php _e( '<strong>Or</strong> any of these <strong>Events</strong>', 'engine' ) ?></label>
</p>
...
<p>
	...
	<label for="<?php echo $show_calendar_button['id'] ?>"><?php _e( 'Show <strong>View Calendar</strong> button', 'engine' ) ?></label>
	<br />
	...
	<label for="<?php echo $show_subscribe_buttons['id'] ?>"><?php _e( 'Show <strong>Subscribe</strong> buttons', 'engine' ) ?></label>
	<br />
	...
	<label for="<?php echo $hide_on_calendar_page['id'] ?>"><?php _e( 'Hide this widget on calendar page', 'engine' ) ?></label>
</p>
